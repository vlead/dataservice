...
class Institute(Entity):
    ...
    def __init__(self, **kwargs):
        ...
        if not isinstance(kwargs['name'], InstituteName):
            raise TypeError("%s is not of type Name" % kwargs['name'])
        ...
        if 'pic' in kwargs:
            if not isinstance(kwargs['pic'], Name):
                raise TypeError("%s is not of type Name" % kwargs['pic'])
            self.pic = kwargs['pic']
              
        if 'iic' in kwargs:
            if not isinstance(kwargs['iic'], Name):
                raise TypeError("%s is not of type Name" %kwargs['iic'])
            self.iic = kwargs['iic']
      ...
      #      raise TypeError("%s is not of type Name" % kwargs['name'])
        ...
        if 'dnc' in kwargs:
            if not isinstance(kwargs['dnc'], Name):
                raise TypeError("%s is not of type Name" % kwargs['dnc'])
            self.dnc = kwargs['dnc']
...
class TestLab(TestCase):
    ...
    def test_lab_get_developers(self):
        ...
        intt = Institute(name="MIT", mnemonic="mit")
        dev1 = Developer(name="Joe", email_id="joe@mit.com", institute=instt)
        dev2 = Developer(name="Poe", email_id="poe@mit.com", institute=instt)
        devs = [dev1, dev2]
    ...
    def test_lab_set_name(self):
        ...
        lab = Lab(name="DS", mnemonic="ds")
        lab.set_name(Name("Data Structures"))
        self.assertEqual(lab.get_name(), "Data Structures")
        self.assertRaises(TypeError, lab.set_name, "Data Structures")
    ...
    def test_lab_set_integration_level(self):
        ...
        self.assertEqual(lab.get_integration_level(), ilevel)
    ...
    def test_lab_set_type_of_lab(self):
        ...
        lab = Lab(name="DS", mnemonic="ds")
        lab_type = TypeOfLab(name="Simulation")
        lab.set_type_of_lab(lab_type)
        self.assertEqual(lab.get_type_of_lab(), lab_type)
...
class TestDeveloper(TestCase):
    ...
    def test_developer_constructor(self):
        ...
        developer = Developer(name=name, email_id=email_id, institute=inst)
        print "developer = %s" % developer
        self.assertEqual(isinstance(developer, Developer), True)
#+END_SRC
