...
class Developer(Entity):
    ...
    email_id = db.Column(db.String(128), unique=True)
...
class Experiment(Entity):
    ...
    content_url = db.Column(db.String(256), unique=True)
    ...
    simulation_url = db.Column(db.String(256), unique=True)
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def to_client(self):
        return {
            ...
            'integration_level': self.integration_level.to_client(),
            'repo_url': self.repo_url,
            'hosted_url': self.hosted_url,
            'hosted_on': self.hosted_on,
            'technologies': [t.to_client for t in self.get_technologies()],
            'experiments': [e.to_client for e in self.get_experiments()],
            'developers': [d.to_client() for d in self.get_developers()],
            'type_of_lab': self.type_of_lab.to_client(),
            'status': self.status,
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def __init__(self, **kwargs):
        ...
        if 'data' not in kwargs:
            raise AttributeRequired("`data` attribute is required.")
        ...
        if not isinstance(kwargs['data'], dict):
            raise TypeError("`data` should be a python `dict`")
        ...
        if 'user_name' in kwargs:
            if not isinstance(kwargs['user_name'], str):
                raise TypeError("user_name should be of type str")
            self.user_name = kwargs['user_name']

        if 'user_email' in kwargs:
            if not isinstance(kwargs['user_email'], str):
                raise TypeError("user_email should be of type str")
...
def query_an_entity(entity):
    (ret_val, entity_class) = validate_entity(entity, request.args.keys())

def validate_entity(entity, attributes):
    if entiry not in entity
...
SQLALCHEMY_DATABASE_URI = 'mysql+oursql://username:password@localhost/dbname'
