...
def download_data():
    ...
    outcsv.writerow([col.name for col in curr_entity.__mapper__.columns])
    [outcsv.writerow([getattr(curr, col.name) for col in curr_entity.__mapper__.columns]) for curr in data]
    outfile.close()

    return redirect(url_for('static', filename='temp/'+filename))
    return 'You requested download for %s' % entity
