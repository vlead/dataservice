...
* Requirements
  ...
  3. REST API is provided for various applications to perform CRUD operations
     on the data.
  4. A bulk update interface, to update bulk data at once. <<req4>>
...
class Institute(Entity):
    ...
    institute_id = db.Column(db.String(16), nullable=False)
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    @staticmethod
    def get_all():
        return [i in Institute.query.all()]
...
class Discipline(Entity):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(128), nullable=False)
    discipline_id = db.Column(db.String(16), nullable=False)
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    @staticmethod
    def get_all():
        return [i in Discipline.query.all()]
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def __init__(self, **kwargs):
        if 'name' not in kwargs:
            raise AttributeRequired("non-nullable attribute name is missing")
        self.set_name(kwargs['name'])

        if 'email_id' not in kwargs:
            raise AttributeRequired("non-nullable attribute email is missing")
        self.set_email(kwargs['email_id'])

        if 'institute' not in kwargs:
            raise AttributeRequired("non-nullable attribute institute is missing")
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def __init__(self, **kwargs):
        ...
        if 'is_web_2_compliant_lab' in kwargs:
            self.set_web_2_compliant(kwargs['is_web_2_compliant_lab'])
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    ...
    def set_web_2_compliant(self, compliant):
...
** REST APIs
   REST APIs form the interface of the data service with which the
   external world interacts.

   The Python code to have the imports, boilerplate code etc. for the API.
...
*** Requirement
   One of the requirement in the application is the bulk update
   interface. <<req4>>
...
.
   ...
   To provide this facility to download and upload CSV files, the data
   service also needs to provide a static page, from where CSV files
   can be downloaded or uploaded.

** REST API - GET - to get and query all information
   The GET REST APIs are designed in a generic way. That is, APIs for
   each entity is not designed and implemented specifically, rather
   three generic API patterns are designed which, presumably, will
   capture all kinds of query patterns.
   ...
   3. /<entity>s/<:id>/<entity>?query_param1=val1&query_param2=val2...

   There are look up tables defined which map keywords to an
   entity. Using this look up table the REST APIs can verify and check
   if a request is valid. Whenever there is an new entity, only the
   look up table has to be modified.

   Look up table would be like:
...
** Authentication by =dataservice=

   This explains how =dataservice= will authenticate other services
   communicating with it.

   + By IP Blocking:

     Here the assumption is that =dataservice= is aware of
     microservices such as =ADS= or =dashboard=.

     According to the IP Blocking technique, =dataservice= will
     identify those IP addresses from which web requests will be
     accepted.  This includes =ADS= and other such microservices.
     Each time a web request is received, =dataservice= will compare
   ...
   + By CORS (Cross Origin Resource Sharing):
     ...
     Since =dataservice= is aware of the microservices, it will allow
     CORS for ajax requests coming *only* from these microservices.
...
class TestLab(TestCase):
    ...
    def test_lab_set_web_2_compliant(self):
        ...
        lab = Lab(name="DS", lab_id="ds")
        lab.set_web_2_compliant(True)
        self.assertEqual(lab.is_web_2_compliant(), True)
        self.assertRaises(TypeError, lab.set_web_2_compliant, "Yes")
...
class TestInstitute(TestCase):
    ...
    def test_get_institute_developers(self):
        ...
        instt = Institute(name="MIT", institute_id="mit")
        dev1 = Developer(name=Name("Alan"), email_id=Email("alan@example.com"), institute=instt)
        dev2 = Developer(name=Name("john"), email_id=Email("john@example.com"), institute=instt)
        devs = [dev1, dev2]
