...
class Institute(Entity):
    ...
    def __init__(self, **kwargs):
        ...
        if not isinstance(self.name, InstituteName):
            raise TypeError

        self.name = name

        if self.mnemonic not in kwargs:
            raise AttributeRequired("mnemonic is required when initializing an institute")

        if self.pic in kwargs:
            if not isinstance(self.pic, Name):
                raise TypeError
            self.pic = pic

        if self.iic in kwargs:
            if not isinstance(self.iic, Name):
                raise TypeError
            self.iic = iic
...
class Discipline(Entity):
    ...
    def __init__(self, **kwargs):
        ...
        if self.dnc in kwargs:
            if not isinstance(self.dnc, Name):
                raise TypeError
            self.dnc = dnc
