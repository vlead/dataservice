...
def typecast_compound_item(const, key, val):
    if 'id' not in val:
        abort(400, "id attr has to be present in %s:%s" % (key,
                                                           val))
    try:
        new_val = const.get_by_id(val['id'])
    except TypeError:
        abort(400, '%s is not a valid %s' % (val, key))

    if not new_val:
        abort(404, 'id %s of %s is not found' % (val['id'], key))
