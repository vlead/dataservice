...
#+BEGIN_SRC python :tangle src/db.py :eval no
    @staticmethod
    def get_all():
        return [i.to_client() for i in Experiment.query.all()]
...
#+BEGIN_SRC python :tangle src/db.py :eval no
    def to_client(self):
        return {
            ...
            'content_hosted_on': self.content_hosted_on,
            'simulation_url': self.simulation_url,
            'simulation_hosted_on': self.simulation_hosted_on,
...
def query_an_entity(entity):
    ...
    return jsonify_list(curr_entity.get_all())
